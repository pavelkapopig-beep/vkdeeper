...
def upload_all_images(vk_session):
    ...
    images = {
        "ask": "ask.jpg", "start": "start.jpg", "img1": "img1.jpg",
        "img2": "img2.jpg", "img3": "img3.jpg", "img4": "img4.jpg",
        "img5": "img5.jpg", "error": "error.jpg", "final": "final.jpg"
    }
...
def send_message(vk, user_id, text, image_key=None):
    ...
    vk.messages.send(user_id=user_id, message=text, random_id=random.randint(1, 999999), attachment=attachment)
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            if uid not in states:
                states[uid] = "menu"
                send_message(vk, uid, "🧙‍♂️ Приветствую тебя любитель физики!\nЧто желаешь?\n1️⃣ Поговорить\n2️⃣ Посвящение\n\nНапиши 1 или 2", "ask")
                continue
            ...
            if states[uid] == "menu":
                if msg in ["1", "поговорить"]:
                    ...
                    init_step[uid] = -1
                    send_message(vk, uid, "Готов произнести клятву и очистить свой ум и разум от ереси?\n\nНапиши ДА или НЕТ", "ask")
                else:
            ...
            if states[uid] == "initiation":
                ...
                # Шаг -1: ожидание ответа ДА или НЕТ
                if step == -1:
                    if msg == "да":
                        init_step[uid] = 0
                ...
                # Шаги 0-5: проверка ответов на клятвы
                if 0 <= step < len(INITIATION_STEPS):
                    current_step = INITIATION_STEPS[step]
                    
                    if current_step["need_confirm"]:
                        # Если пользователь сказал "клянусь" - переходим к следующему шагу
                        if msg == "клянусь":
                            next_step = step + 1
                            if next_step < len(INITIATION_STEPS):
                                init_step[uid] = next_step
                                send_message(vk, uid, INITIATION_STEPS[next_step]["text"], INITIATION_STEPS[next_step]["image"])
                            else:
                                # Все клятвы произнесены! Посвящение завершено
                                states[uid] = "chat"
                                send_message(vk, uid, "🎉 ПОЗДРАВЛЯЮ! ТЫ ПРОШЁЛ ПОСВЯЩЕНИЕ! 🎉\n\nГимн: https://www.youtube.com/watch?v=UPSDSKaVXA8\nПодпишись: https://vk.com/club238310497", "final")
                        else:
                            # Если сказал что-то другое - ругаемся, НО НЕ СБРАСЫВАЕМ ШАГ
                            # Пользователь может снова сказать "клянусь" и продолжить с того же места
                            send_message(vk, uid, "Ты мне втираешь какую-то дичь!\nЯ тебя проклинаю, бес!\n\nСкажи КЛЯНУСЬ, чтобы продолжить!", "error")
                    continue
